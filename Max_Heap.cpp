...
class HeapTree
{
    ...
    void InsertDataInHeap(int data)
    {
        ...
        else
        {
            ...
            CreateHeap(length);//length = index
            length++;    
        }   
    }  
    //display heap
    ...
    //maximum number = first index in max heap
    int MaximumNumber()
    {
        return heapArray[0];
    }
    ...
    void DeleteHeap(int index)
   {
        ...
        int temp = index; 
        //check the left child is greater or not
        if(left < length && heapArray[left] > heapArray[temp])
        {
            temp = left;
        }
        //check the right child is greater or not
            ...
            while (choice != -1)
            {
                ...
                switch (choice)
                {
                    ...
                    case 4:
                        ...
                        break; 
                    case 5:
                        cout << "Element are deleted in max heap-->\t" << tree.remove() << endl << endl; 
                        tree.DisplayHeap();   
                        break;
                    case -1:
